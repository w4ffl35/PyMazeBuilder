...
class StairsGenerator(Generator):
    def __init__(
        self, data: Optional[dict] = None, ascending: bool = False, max_stairs: int = 1
    ):
        self.data = data or {}
    ...
    def generate(self):
        ...
        for floor in range(self.data["grid"].total_floors - 1):
            ...
            while True:
                if (
                    floor in total_stairs_by_floor
                    and total_stairs_by_floor[floor] >= self.max_stairs
                ):
                    break
                ...
                if floor > 0:
                    previous_floor_cell = self.data["grid"].cells[floor - 1][cell.y][
                        cell.x
                    ]
                ...
                if (
                    next_floor_cell is None
                    or next_floor_cell.blocked
                    or next_floor_cell.stairs
                ):
                    continue
